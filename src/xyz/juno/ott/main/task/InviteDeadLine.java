...
public class InviteDeadLine extends BukkitRunnable {
	private BukkitTask bukkitTask;
	private Player p;
	private static Map<Player, Integer> cooldownMap = Collections.synchronizedMap(new HashMap<Player, Integer>());
	
	public InviteDeadLine(Player p, int cooldown, JavaPlugin javaPlugin, boolean asynchronously) {
		this.p = p;
	...
	public void overRun() {
		...
		switch (cooldownMap.get(p)) {
			case 0:
				OTT.inviteDeadlineManager().stopCooldown(p);
				DataPlayer.INVITE_DATA.remove(p);
				break;
		}
	}
}